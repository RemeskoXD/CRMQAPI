...
export function createReportRoutes(repos: Repositories): Router {
  const router = Router();
  router.use(authenticate);
  router.use(authorize('reports:read', 'dashboard:read'));
  ...
  router.get('/overview', async (_req, res) => {
    try {
      ...
      projects.forEach(p => {
        ...
        if (p.totalPrice) revenueByType[p.type] = (revenueByType[p.type] || 0) + p.totalPrice;
      });
      ...
      projects.forEach(p => {
        ...
        if (p.totalPrice) revenueBySource[src] = (revenueBySource[src] || 0) + p.totalPrice;
      });
      ...
      const invoicesByType: Record<string, { count: number; total: number }> = {};
      invoices.forEach(i => {
        if (!invoicesByType[i.type]) invoicesByType[i.type] = { count: 0, total: 0 };
    ...
    try {
      ...
      switch (entity) {
        ...
        case 'projects': {
          ...
          const typeLabels: Record<string, string> = { garage_doors: 'Vrata', windows: 'Okna', shading: 'Stínění' };
          const statusLabels: Record<string, string> = { new_inquiry: 'Poptávka', site_visit: 'Obhlídka', pricing: 'Nacenění', waiting_material: 'Materiál', in_progress: 'Montáž', done: 'Hotovo', invoiced: 'Fakturováno' };
          data = data.map(p => {
        ...
        case 'invoices': {
          ...
          const typeL: Record<string, string> = { quote: 'Nabídka', advance: 'Záloha', invoice: 'Faktura' };
      ...
      if (format === 'csv') {
        ...
        const htmlRows = data.map(row =>
          `<tr>${headers.map(h => `<td style="padding:6px 10px;border:1px solid #ddd;">${String(row[h] ?? '')}</td>`).join('')}</tr>`
        ).join('');
...
function getTopClients(clients: any[], projects: any[], limit: number) {
  const map: Record<string, { name: string; projects: number; revenue: number }> = {};
  clients.forEach(c => {
...
function sanitizeXml(s: string) { return s.replace(/[^a-zA-Z0-9_]/g, '_'); }
