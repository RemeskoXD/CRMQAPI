...
function createDashboardRoutes(repos) {
    ...
    router.get('/', async (req, res) => {
        try {
            ...
            if (role === 'technician') {
                ...
                stats.todayTasks = myTasks.filter(t => t.dueDate?.startsWith(todayStr) && t.status !== 'done' && t.status !== 'cancelled');
                stats.upcomingTasks = myTasks
                    .filter(t => t.status !== 'done' && t.status !== 'cancelled')
                    .sort((a, b) => (a.dueDate ?? '').localeCompare(b.dueDate ?? ''))
                    .slice(0, 10);
                stats.completedToday = myTasks.filter(t => t.completedAt?.startsWith(todayStr)).length;
